module Unitsml
  module IntermediateExpRules
    ...
    rule(:prefixes_units_named_exp) do
      prefixes_units |
        ((str("(").as(:open_parenthesis) >> prefixes_units_named_exp.as(:int_exp) >> str(")").as(:close_parenthesis)).as(:intermediate_exp) >> extended_prefixed_units.maybe)
    end
    ...
    rule(:dimension_rules_named_exp) do
      dimension_rules |
        ((str("(").as(:open_parenthesis) >> dimension_rules_named_exp.as(:int_exp) >> str(")").as(:close_parenthesis)).as(:intermediate_exp) >> extended_dimension_rules.maybe)
    end
