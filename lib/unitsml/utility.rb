    ...
    class << self
      ...
      def unitname(units, text, name)
        Model::Units::Name.new(
          name: (unit_instance(text) ? Unit.new(text).enumerated_name : text)
        )
      end
      ...
      def dimension(norm_text)
        return unless unit_instance(norm_text)&.dimension_url

        dim_id = unit_instance(norm_text).dimension_url.sub("#", '')
        dim_attrs = { xmlns: UNITSML_NS, id: dim_id }
      ...
      def dimension_components(dims)
        ...
        dim_attrs = { xmlns: UNITSML_NS, id: dim_id(dims) }
