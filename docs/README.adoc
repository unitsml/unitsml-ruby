...
A checked-in `unitsdb/` directory is retained for static/Opal fallback loading.

== Development setup

During local development, this repository expects `lutaml-model`, `mml`,
`plurimath`, and `unitsdb-ruby` to be available as sibling checkouts at
`../lutaml-model`, `../mml`, `../plurimath`, and `../unitsdb-ruby`. The
development `Gemfile` uses Bundler `path` dependencies for those repositories.

The standard Ruby runtime path goes through `::Unitsdb.database`. When a local
`unitsdb-ruby` checkout does not have its `data/` submodule populated, UnitsML
falls back to the packaged YAML files under `vendor/unitsdb`. The checked-in
`unitsdb/` YAML files are reserved for the Opal/static fallback in
`lib/unitsml/opal_unitsdb.rb`.
