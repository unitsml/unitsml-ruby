...
Default value of `explicit_parenthesis` is `true`, which means the parsed parentheses will be included in the output appropriately.
[source,ruby]
----
unit = Unitsml.parse("(m/s)")
...
Setting the `explicit_parenthesis` to `false` will omit the parentheses in the output:
...
NOTE: Parentheses around unit expressions (e.g., `(m/s)`) are parsed and their inclusion in the output can be controlled. However, nested parentheses around exponents are not supported. For instance, `m/s^(2)` is valid, but `m/s^((2))` is not and will result in a parsing error.
