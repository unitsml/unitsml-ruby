...
its `short` name (e.g. `"watt"`). An unknown unit, prefix or a units/dimensions
mix raises a `Unitsml::Errors::*` error.

The `Unitsml.compose` keyword form builds the same `Formula` from a list, where
each entry is a symbol id, a `short` name, a `{ unit:, power:, prefix: }` hash,
or a pre-built `Unitsml::Unit`:
