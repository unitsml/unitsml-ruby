...
A unit is referenced by its symbol id (as in a parsed string, e.g. `"W"`) or by
its `short` name (e.g. `"watt"`). Dimensions compose the same way
...
`short` name, a `{ unit:, power:, prefix: }` hash, or a pre-built
`Unitsml::Unit`; a `dimensions:` entry is a dimension id or a
`{ dimension:, power: }` hash:
...
`Unitsml.compose` validates every reference, power and prefix it is given and
raises a `Unitsml::Errors::*` (all subclasses of `Unitsml::Errors::BaseError`)
for anything it cannot resolve: an unknown unit, dimension or prefix, a
units/dimensions mix, an empty composition, or a power the parser cannot express
(a decimal exponent — use a `Rational`). The operator DSL composes the objects
you construct, so — as with rendering a `Unit` or `Dimension` directly — a
hand-built object carrying an unresolvable prefix or dimension name is the
